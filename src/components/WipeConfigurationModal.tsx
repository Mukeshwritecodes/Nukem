    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="text-center pb-6">
          <DialogTitle className="text-[#024950] text-4xl font-bold mb-2">
            Wipe Configuration
          </DialogTitle>
          <p className="text-[#0FA4AF] text-xl">
            Customize your secure data wipe
          </p>
        ...
        <div className="space-y-8">
                ...
                <button
                  ...
                  className={`flex flex-col items-center p-6 rounded-lg border-2 transition-all ${
                    selectedOS === os.id
                      ...
                      : 'border-[#024950] hover:border-[#0FA4AF]'
                  }`}
                >
                  <div className="text-4xl mb-2">{os.icon}</div>
                  <span className={`text-lg font-medium ${
                    selectedOS === os.id ? 'text-[#0FA4AF]' : 'text-[#024950]'
                  }`}>
                    {os.name}
                  </span>
                </button>
                ...
                {wipeTypes.map((type) => (
                  <div key={type.id} className="flex items-center space-x-3">
                    <RadioGroupItem 
                      value={type.id} 
                      id={type.id}
                      className="border-[#024950] text-[#0FA4AF]"
                    />
                    <Label 
                      htmlFor={type.id} 
                      className="text-[#0FA4AF] text-lg font-medium cursor-pointer"
                    >
                      {type.name}
                    </Label>
                  </div>
            ...
            <div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    id="metadata-purge"
                    checked={metadataPurge}
                    onCheckedChange={setMetadataPurge}
                    className="data-[state=checked]:bg-[#0FA4AF]"
                  />
                </div>
                <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col gap-4 pt-6">
            <Button
              ...
              className="w-full h-14 bg-[#0FA4AF] hover:bg-[#0A8CBF] text-white text-lg font-bold rounded-xl"
              disabled={!selectedOS}
            >
            ...
            <Button
              ...
              variant="outline"
              className="w-full h-14 border-[#0FA4AF] text-[#0FA4AF] hover:bg-[#0FA4AF] hover:text-white text-lg font-bold rounded-xl"
              disabled={!selectedOS}
            >
